...
polygon_shape <- reactive({
  e <- terra::ext(input$west_coord, input$east_coord, input$south_coord, input$north_coord)
...
raster_mean <- reactive({
  mean_values <- terra::extract(summary_raster_model()[[start_year():end_year()]], polygon_shape(), mean, na.rm = TRUE, ID = FALSE)
