...
user_created_map <- reactive({
  # If the map is toggled to "Imagery" and "raster" layers, show those layers in the download
  if (identical(x = c("Imagery", "raster"), y = input$map_groups)) {
    ...
    user_created_map <- foundational_map() %>%
      ...
      leaflet::showGroup(group = c("Imagery", "raster"))
    
    return(user_created_map)
    
    # Else if the map is toggled to "Grey background" and "raster" layers, show those layers in the download
  } else if (identical(x = c("Grey background", "raster"), y = input$map_groups)) {
    # Call the foundational Leaflet map
    user_created_map <- foundational_map() %>%
      # Store the view based on UI
      leaflet::setView(
      ...
      # Show only the layer(s) the user toggled to
      leaflet::showGroup(group = c("Grey background", "raster"))
    ...
    user_created_map <- foundational_map() %>%
      ...
      # Show only the layer(s) the user toggled to
      leaflet::hideGroup(group = c("raster")) %>%
      leaflet::showGroup(group = c("Imagery"))
    
    return(user_created_map)
    
    # Else if the map is toggled to only the "Grey background" layer, show only that layer in the download
  } else if (input$map_groups == "Grey background") {
    # Call the foundational Leaflet map
    user_created_map <- foundational_map() %>%
      # Store the view based on UI
      leaflet::setView(
      ...
      # Show only the layer(s) the user toggled to
      leaflet::hideGroup(group = c("raster")) %>%
      leaflet::showGroup(group = c("Grey background"))
